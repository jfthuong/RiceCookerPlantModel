...
REM
REM  Output: simulink\model\RiceCookerPlant.slx
REM          simulink\model\RiceCookerWithPanel.slx
REM
...
pushd "%SIMULINK_DIR%"
"%MATLAB_EXE%" -batch "addpath('model'); build_model; build_model_with_panel"
set EXIT=%ERRORLEVEL%
popd
...
if not exist "%SIMULINK_DIR%\model\RiceCookerPlant.slx" (
    echo ERROR: RiceCookerPlant.slx not produced.
    goto Error
)

if not exist "%SIMULINK_DIR%\model\RiceCookerWithPanel.slx" (
...
echo Outputs: model\RiceCookerPlant.slx and model\RiceCookerWithPanel.slx
echo.
endlocal
timeout /t 20
exit /b 0